...
There was a decision made early to change the Minimum Viable Product (MVP) to have recipes in textual form rather than as videos and to avoid machine learning for generating recommended recipes. The project's focus is to deliver all MVP features as planned in the original proposal. However, after a close review during team discussions, it was identified that many quality attributes lacked sufficient justification or relevance to the goals of the project at its current stage. As a result, the team mutually agreed to refine the quality attributes to include only the three most critical and applicable characterisitics which are; Availability, Extensibility and Scalability. These attributes were selected based on their direct impact on the system's reliability, long-term adaptibility and capacity to serve users efficiently under varying loads.
...
The architecture we chose for Chef de Cuisine supports the main goals of our project very well. First, using microservices means that each part of the system, such as the search function, filtering service, and user management, works as an independent service. If there is a problem with one service, the other parts can still work as usual. This is very important because users might be cooking and need to find a recipe quickly, so we want the platform to be reliable and reduce the risk of total failure. Also, the modular design of microservices makes it easier to add new features in the future. For example, if we want to add a meal planner or nutrition tracker, we can create a new service without changing too much of the existing code. This helps avoid many bugs and makes teamwork more efficient, since different team members can work on different services at the same time. Furthermore, by using container technology and deploying our system in the cloud, we can let each service handle more traffic independently, especially during lunch and dinner times when more users visit the site. However, this architecture also brings some new challenges, such as needing more work to monitor, test, and manage resources, because there are more services to keep track of. Compared with a simple monolithic structure, microservices are more complex, but they give us better flexibility and future growth. We think this is a good trade-off, because it matches our project’s needs and supports further development.
